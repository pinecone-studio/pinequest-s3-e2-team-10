...
async function proxyRequest(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt += 1) {
    ...
    try {
      const upstreamResponse = await fetch(buildUpstreamUrl(path, request.url), {
      ...
      if (attempt === RETRY_DELAYS_MS.length) {
        return Response.json({ message: 'Backend сервертэй холбогдож чадсангүй.' }, { status: 503 })
      }
    } finally {
  ...
  return Response.json({ message: 'Backend сервер одоогоор ажиллахгүй байна.' }, { status: 503 })
}
