        ...
        {launchExams.length === 0 ? (
          ...
          <div className="mt-5 overflow-hidden rounded-[26px] border border-[#e5edff] bg-[#fbfdff]">
            ...
            {launchExams.map((exam) => (
              <article key={exam.id} className="border-b border-[#e5edff] px-5 py-4 last:border-b-0">
                <div className="grid gap-4 md:grid-cols-[minmax(0,1.7fr)_120px_160px_150px] md:items-center">
                  <div>
                    <h3 className="text-base font-semibold text-[#303959]">{exam.title}</h3>
                    <p className="mt-1 text-sm text-[#6f7898] md:hidden">{exam.questions.length} асуулт • {getLaunchStatusLabel(exam)}</p>
                  </div>
                  <div className="hidden text-sm text-[#6f7898] md:block">{exam.questions.length} асуулт</div>
                  <div className="hidden text-sm text-[#6f7898] md:block">{getLaunchStatusLabel(exam)}</div>
